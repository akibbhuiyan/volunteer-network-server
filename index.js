...
async function run() {
  try {
    const activytyCollection = client
    ...
    app.post("/volunteer", (req, res) => {
      ...
      console.log(newUser);
      volunteerCollection.insertOne(newUser).then((result) => {
    ...
    app.post("/addevent", (req, res) => {
      const newUser = req.body;
      console.log(newUser);
      activytyCollection.insertOne(newUser).then((result) => {
        res.send(result.acknowledged);
      });
    });
    ...
    app.get("/admintask/delete", (req, res) => {
      ...
      console.log(remove);
      volunteerCollection.deleteOne({ _id: remove }).then((result) => {
        res.send(result);
      });
    });
